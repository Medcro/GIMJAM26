...
func enter():
	...
	if not book.left_title.meta_clicked.is_connected(_on_link_clicked):
		book.left_title.meta_clicked.connect(_on_link_clicked)
	...
	if not book.right_title.meta_clicked.is_connected(_on_link_clicked):
		book.right_title.meta_clicked.connect(_on_link_clicked)
	if not book.right_lable.meta_clicked.is_connected(_on_link_clicked):
		book.right_label.meta_clicked.connect(_on_link_clicked)
	if not book.right_lable_2.meta_clicked.is_connected(_on_link_clicked):
